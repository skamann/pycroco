...
class CrossCorrel(object):
    ...
    def add(self, spectrum, template=False, id=None, continuum_mode=ContinuumMode.SigmaClipping, continuum_kwargs=None):
        ...
        if not isinstance(spectrum, Spectrum):
            raise IOError("Only instances of spexxy.data.Spectrum can be added.")
        ...
        # new_frame = pd.DataFrame({'wave': spectrum.wave, 'flux': spectrum.flux / continuum - 1.,
        #                           'mask': ~spectrum.valid if spectrum.valid is not None else np.zeros(len(spectrum), dtype=bool)})
    ...
    def correlate(spectrum, template, stepsize=1, offset=0, velocity=False):
        ...
        corr12 = np.correlate(a, v, "full")

        # get abscissa for correlation function (incl. right zero point)
        xdata = np.arange(corr12.size, dtype=np.float64)
        xdata -= v.size - 1  # when self-correlating an array, the peak is at pixel len(array)-1
    ...
    def fit_peak(self, signal, search_center=None, search_width=None, fit_width=None, function=FitFunction.Gauss):
        ...
        x2 = np.sum(x_cc[center_pixels] * x_cc[center_pixels] * y_cc[center_pixels]) / total - x ** 2
    ...
    def __call__(self, fftfilter=False, filterpars=None, apodize=0.2, search_width=1000., search_center=0,
                 fit_peak=True, fit_width=200., fit_function=FitFunction.Gauss, full_output=False,
                 **kwargs):
        """
        ...
        Parameters
        ----------
        fftfilter : boolean, optional
            If set to true, the continuum subtracted spectra are multiplied
            with a ramp-filter in fourier space.
        filterpars : array_like, optional
            The parameters of the ramp-filter: cuton, fullon, fulloff, cutoff
            See http://arxiv.org/pdf/0912.4755.pdf for meaning of parameters
            Only has an effect if 'fftfilter' flag is set to true.
        apodize : float, optional
            The fraction at the edge of each valid range that is apodized with
            a cosine filter prior to Fourier filtering.
        ...
        for template_id, _template in self.templates.items():
            ...
            if not _template.wave_mode == Spectrum.Mode.LOG10LAMBDA:
                _template.mode(Spectrum.Mode.LOG10LAMBDA)

            # use constant sampling in log-space
            if _template.wave_step == 0.:
                template = _template.resample_const()
            ...
            if fftfilter:
                ...
                template.flux = self.apodize_edges(template.flux, template.valid, apodize=apodize)

                #  Use default values for Fourier filter parameters is none provided
                self.logger.info('... FFT-filtering with parameters {0} ...'.format(filterpars))
                try:
                    tmp = ramp(template.flux, filterpars[0], filterpars[1], filterpars[2], filterpars[3])
                except ValueError:
            ...
            for spectrum_id, _spectrum in self.spectra.items():
                ...
                _spectrum.mode(Spectrum.Mode.LOG10LAMBDA)
                ...
                if fftfilter:
                    # mask out invalid regions and apodize edges
                    spectrum.flux = self.apodize_edges(spectrum.flux, spectrum.valid, apodize=apodize)

                    #  Use default values for Fourier filter parameters is none provided
                    self.logger.info('... FFT-filtering with parameters {0} ...'.format(filterpars))
                    try:
                        tmp = ramp(spectrum.flux, filterpars[0], filterpars[1], filterpars[2], filterpars[3])
                    except ValueError:
                        self.logger.error('... FFT-filtering failed')
                    else:
                        spectrum.flux = tmp
                ...
                print(spectrum.flux, final_spectrum_flux)
        ...
        if full_output:
            return self.results, self.cc_functions
        else:
