...
class Gauss(object):
    ...
    @staticmethod
    def profile(x, x0, peak, sigma, continuum=0):
        """
        ...
        sigma : float
            The standard deviation of the Gaussian in units of pixels.
        ...
        """
        return continuum + peak*np.exp(-0.5*(((x - x0)/sigma)**2))

    @staticmethod
    def fwhm(x0, peak, sigma, **kwargs):
        # if kwargs:
        #     raise IOError("Unknown parameter(s) provide: {}".format(kwargs))
        return 2.355 * sigma

    @staticmethod
    def vrad(x0, peak, sigma, **kwargs):
        # if kwargs:
        #     raise IOError("Unknown parameter(s) provide: {}".format(kwargs))
        return x0
...
class Moffat(object):
    ...
    @staticmethod
    def profile(x, x0, peak, r0, beta=2.5, continuum=0):
        """
