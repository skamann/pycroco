...
def plot_cc_data(cc_data: CCData, v: float = None, verr: float = None, peak_function: CrossCorrel.FitFunction = None,
    ...
    ax_input.plot(cc_data.spectrum.index, cc_data.spectrum.values, ls='-', lw=1.5, c='C0', marker='None')
    ax_input.plot(cc_data.template.index, cc_data.template.values, ls='-', lw=1.5, c='C4', marker='None')

    ax_input.set_xlabel(r'Wavelength ($\AA$)')
